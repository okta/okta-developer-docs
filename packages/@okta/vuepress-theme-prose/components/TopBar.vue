<template>
<div>
  <nav class="top-bar-new container">
          <a href="https://developer.okta.com/" class="logo">
          ...
          <nav :class="{'header-nav':true, 'expanded': !topBarCollapsed}">
            <ul class="menu">
              <li v-for="(link, index) in $themeConfig.primary_nav" :key="index" :class="{'hasSubMenu': link.children}">
                ...
                <ul v-if="link.children" class="submenu">
                  <li v-for="(childLink, childIndex) in link.children" :key="childIndex">
                    <a v-if="childLink.link" :href="childLink.link" class="link">{{childLink.text}}</a>
                    <div v-else-if="childLink.text" v-html="childLink.text" class="non-link"></div>
                    <div v-else-if="childLink.type === 'divider'" class="divider"></div>
                  </li>
                </ul>
              </li>
            </ul>
              ...
              <a class="buttons-block--button red" href="https://developer.okta.com/signup/" >Sign Up</a>
            </div>
          </nav>
          <div class="top-bar-new--controls">
           <button v-if="topBarCollapsed" v-on:click="toggleSearch()" :class="{'search-icon': true, 'active': showSearchBar}"></button>
           <div class="top-bar-new--controls--toggle" @click="topBarCollapsed = !topBarCollapsed">
              <svg viewBox="0 0 20 20" class="toggle" v-if="topBarCollapsed">
                <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z"/>
              </svg>
              <svg viewBox="0 0 352 512" class="toggle-close" v-else>
                <path d="M242.72 256l100.07-100.07c12.28-12.28 12.28-32.19 0-44.48l-22.24-22.24c-12.28-12.28-32.19-12.28-44.48 0L176 189.28 75.93 89.21c-12.28-12.28-32.19-12.28-44.48 0L9.21 111.45c-12.28 12.28-12.28 32.19 0 44.48L109.28 256 9.21 356.07c-12.28 12.28-12.28 32.19 0 44.48l22.24 22.24c12.28 12.28 32.2 12.28 44.48 0L176 322.72l100.07 100.07c12.28 12.28 32.2 12.28 44.48 0l22.24-22.24c12.28-12.28 12.28-32.19 0-44.48L242.72 256z"/>
              </svg>
            </div>
          </div>  
  </nav>
  <div class="searchbar" v-show="showSearchBar && topBarCollapsed">
              <Search />
            </div>
</div>
</template>
