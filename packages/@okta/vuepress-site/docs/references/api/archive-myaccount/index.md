...
> **Note:** This version of the MyAccount API is deprecated. For the latest version of the API, see the [reference documentation](/docs/references/api/myaccount/).

<ApiLifecycle access="deprecated" />

The Okta MyAccount API allows end users (with or without administrator access) to fetch and update their own Okta user profiles.  It implements a subset of the existing [Users API](/docs/references/api/users/) but with significant differences.  This API does not expose information an end user should not have access to, and it does not support lifecycle operations.
