...
This page provides the API rate limits for authentication and end-user activities, which is part of Okta [rate limits](/docs/references/rate-limits). To learn more about rate limits, visit our [Overview](/docs/references/rate-limits) and [Best practices](/docs/references/rl-best-practices) pages.
...
See the following list of **per-minute** limits. If an endpoint is not in this list, you can review it using the Admin Console, in the rate limit dashboard's APIs table. See [APIs table](/docs/references/rl-dashboard/#apis-table).
