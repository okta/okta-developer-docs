...
* The Create a group push mapping (`POST /api/v1/apps/{appId}/group-push/mappings`) didn't return validation errors when requests included null attributes or incompatible Active Directory configurations. (OKTA-1061899)
...
| [Passkeys from Android devices is GA in Preview](#passkeys-from-android-devices-is-ga-in-preview) | July 16, 2025 |
...
| [OIN Wizard UI improvement](#oin-wizard-ui-improvement) | November 5, 2025 |
...
#### Passkeys from Android devices is GA in Preview
...
See [Customize associated domains](/docs/guides/custom-well-known-uri/main/). <!-- ASSOCIATED_DOMAIN_CUSTOMIZATION OKTA-935558 preview org date: July 16, 2025 -->

#### New claims supported for identity verification (IDV) vendor integration is now GA in Preview
...
Use the [Replace an authenticator method](https://developer.okta.com/docs/api/openapi/okta-management/management/tag/Authenticator/#tag/Authenticator/operation/replaceAuthenticatorMethod) endpoint to update your WebAuthn authenticator, or set the custom RP ID in the Admin Console. See [Configure a relying party ID](https://help.okta.com/okta_help.htm?type=oie&id=customize-relying-party-id). <!-- OKTA-1008019 WEBAUTHN_CUSTOM_RP_ID preview org date: Sep 4, 2025 -->
...
Admins can now quickly integrate Universal Logout-enabled apps using Express Configuration. When Universal Logout is available for an Express Configuration app, a **Configure SSO & UL** button appears on the configuration page. See [Add an app with Express Configuration](https://help.okta.com/okta_help.htm?type=oie&id=express-configuration). <!-- EXPRESS_CONFIGURATION_FOR_AUTH0_APPS_PHASE3. OKTA-1046722 -->
...
#### OIN Wizard UI improvement

In the OIN Wizard, the `Configure your integration` step is now two separate steps: `Add integration details` and `Configure your integration`. This makes the integration steps more manageable and easier for users to complete. <!-- INTEGRATION_DETAILS_SUBMISSION OKTA-1007247 -->
...
#### Claims sharing with device claims

You can now use shared device claims to satisfy advanced policy conditions on the service provider (SP) org. Ensure that access is uniformly granted only to devices that meet your org's required state, regardless of which Okta org performs the initial authentication. Share device claims simplifies admin overhead for secure federation and eliminates redundant device posture prompts, which delivers a seamless SSO experience for users moving between federated domains. <!-- ORG2ORG_CLAIMS_SHARING_WITH_DEVICE_CLAIMS OKTA-857301 -->
...
| Change | Expected in Preview Orgs |
|--------|--------------------------|
| [Bug fixed in 2025.08.3](#bug-fixed-in-2025-08-3)| August 27, 2025 |
...
| [System Log event for Identity Assertion Authorization Grant (ID-JAG) issuance is EA in Preview](#system-log-event-for-identity-assertion-authorization-grant-id-jag-issuance-is-ea-in-preview) | August 7, 2025 |
...
The `app.oauth2.token.grant.id_jag` event is generated when an app completes an OAuth 2.0 token exchange to get an Identity Assertion Authorization Grant (ID-JAG) JWT. <!-- OKTA-957287 -->
