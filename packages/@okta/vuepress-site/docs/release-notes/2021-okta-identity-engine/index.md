...
The [`login_hint`](/docs/references/api/oidc/#request-parameters) didn't appear in the **Username** box even though the admin had already set up the sign-in flow with the [Org2Org](/docs/concepts/multi-tenancy/#diagram) OIDC [app](/docs/references/api/apps/). (OKTA-445843)
...
Admins can't update a [Profile Enrollment policy](/docs/references/api/policy/#profile-enrollment-policy) by setting the `access` property to `DENY` in the [Profile Enrollment Action object](/docs/references/api/policy/#profile-enrollment-action-object). <!--OKTA-442998-->
...
When the [Update User endpoint](/docs/references/api/users/#update-user) or the [Change Recovery Question endpoint](/docs/references/api/users/#change-recovery-question) was used to update the user's security question, Okta Identity Engine still returned the old security question. (OKTA-442243)
...
Calls to the [List devices](/docs/references/api/devices/#usage-example-expand-user) endpoint with an `expand=user` query now return the management status associated with each embedded user.<!--OKTA-431007-->
...
You can now remove "Powered by Okta" and "© 2021" from the Okta-hosted sign-in page using either the [Customizations > Footer](https://help.okta.com/okta_help.htm?type=oie&id=ext-custom-footer) option in the Admin Console or the [Brands API](/docs/references/api/brands/#brand-api-objects). <!--OKTA-424736-->
...
The [Devices API](/docs/references/api/devices/) allows you to retrieve a single device or a list of devices from an org. The API also allows you to activate, deactivate, suspend, unsuspend, and delete a device by ID. You can access the OAuth 2.0 supported Devices API by using the `okta.devices.manage` and `okta.devices.read` [scopes](/docs/guides/implement-oauth-for-okta/main/#scopes-and-supported-endpoints). <!--OKTA-424362-->

#### New Policy Authenticator settings are available in the Policy API

[Policy Authenticator](/docs/references/api/policy/#policy-authenticator-object) configurations are now included in the Policy API's [Multifactor (MFA) Enrollment Policy](/docs/references/api/policy/#multifactor-mfa-enrollment-policy) settings. You can configure either [Authenticators](/docs/references/api/policy/#policy-authenticators-settings-example) or [Factors](/docs/references/api/policy/#policy-factors-settings-example) for an MFA Enrollment Policy with this API update. The Policy Authenticator configurations are available for all active Authenticators, including enrollment and recovery Authenticators. <!--OKTA-429994-->
