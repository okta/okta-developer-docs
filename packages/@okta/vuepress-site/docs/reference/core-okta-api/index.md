...
| `co`       | contains              | If the operand value exists as a substring in the attribute value, there's a match. This includes the case where the attribute and operand values are identical. This operator is only usable with the [Groups](/docs/reference/api/groups/#list-groups-with-search), [System Log](/docs/reference/api/system-log/#filtering-results), and [Users](/docs/reference/api/users/#list-users-with-search) APIs.                      |
