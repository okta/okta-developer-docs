...
| `operationRateLimitType`           | String | The type of rate limit event affected. Example types: `web_request`, `authenticator_otp_verification`, `sms_factor_enroll`, `event_hook_delivery`, `elastic_rate_limit_activated`, `phone_enrollment`, `profile_reload`<ApiLifecycle access="ie" /><br>, and so on|
...
| `AD agent` <ApiLifecycle access="ie" /><br>       | The user exceeded their limit for profile reload via AD agent       |
| `LDAP agent` <ApiLifecycle access="ie" /><br>     | The user exceeded their limit for profile reload via LDAP agent     |
