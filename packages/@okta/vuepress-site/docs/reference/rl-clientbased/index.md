...
If an org's client-based limit was previously set to **Enforce limit and log per client (recommended)** mode, the setting remains as is. For this mode setting, admins need to monitor syslog events for [`system.client.rate_limit.notification`](/docs/reference/rl-system-log-events/#web-request-rate-limits-org-level) and [`system.client.concurrency_rate_limit.notification`](/docs/reference/rl-system-log-events/#web-request-rate-limits-org-level) in case of client violations. If admins see these one-time notifications from a handful of different users, then those users may be doing something scripted or automated. In such cases, admins can choose to either investigate the issue or choose not to take action. However, if admins see widespread notifications from a large number of users, then it is likely that the application may have an issue, in which case admins need to troubleshoot and make changes to the application where needed. If admins don't see any of those events in the system log, then no action is needed.
