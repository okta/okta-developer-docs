...
As of October 12, 2021, the default client-based rate limit for the OAuth 2.0 `/authorize` and `/login/login.htm` endpoints is set to **Log per client** mode. This means that if an org's client-based rate limit was previously set to **Do nothing**, the setting is changed to **Log per client** mode.
