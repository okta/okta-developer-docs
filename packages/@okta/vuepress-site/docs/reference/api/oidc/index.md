...
| prompt                                         | Valid values: `none`, `enroll_authenticator` <ApiLifecycle access="ea" />, `consent`, `login` or `consent` and `login` in either order. See [Parameter details](#parameter-details) for more information.                                                                                                                                                                                                                                                                                                                                          | Query       | String    | FALSE      |
...
| enroll_amr_values <ApiLifecycle access="ea" /> | Valid values: `pwd`, `kba`, `email`, `sms`, `tel`, `duo`, `symantec`, `google_otp`, `okta_verify`, `pop`, `oath_otp`, `rsa`, `yubikey`, `otp` and `fed`. See [Parameter details](#parameter-details) for more information.                                                                                                                                                                                                                                                                                                                              | Query       | String    | FALSE      |
...
* `prompt`:
  ...
  * `enroll_authenticator` <ApiLifecycle access="ea" />: This indicates that the intent is to enroll the user with an authenticator. The following other parameters must be used together with this value for a valid request:
    * `enroll_amr_values`: This parameter must be specified and indicates which authenticator method you're allowing the user to enroll in.
    * `response_type`: This parameter must be `none`, which means no tokens should be returned at the end of the flow.
    * `acr_values`: This parameter must be `urn:okta:loa:2fa:any:ifpossible`, which means the user will be prompted for at least one factor before enrollment.
    * `max_age`: This parameter must be `0`, which means no existing session should be considered.
    * `scope` and `nonce` must not be specified, because no tokens are generated.

* `enroll_amr_values` <ApiLifecycle access="ea" />: A space-delimited list of values indicating which authenticators to enroll in.
  * If `enroll_amr_values` parameter is specified, then `prompt` must have the value `enroll_authenticator`.
  * The parameter value is space delimited, i.e. `pwd sms okta_verify` is a valid request parameter value. You will be prompted in the order of the amr values provided.
