...
For Public clients (such as single-page and mobile apps), which don't have a `client_secret`, you must include the `client_id` as a query parameter while calling the `/introspect` endpoint. Ensure that you are not passing any Authorization header in the request.
