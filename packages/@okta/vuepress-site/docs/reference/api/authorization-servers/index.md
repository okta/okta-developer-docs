...
| grantTypes   | Array of grantTypes that this condition includes. Accepted grantTypes: `authorization_code`, `client_credentials`, `interaction_code`, `password`, `refresh_token` or . Determines the mechanism that Okta uses to authorize token creation. | `include` list         | True                          |
