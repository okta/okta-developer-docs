...
The rate limit for a user to activate one of their OTP-based factors (such as SMS, CALL, EMAIL, Google OTP, and Okta Verify TOTP) is 5 attempts within 5 minutes. The following example error message is returned if the user exceeds their OTP-based factor rate limit:
