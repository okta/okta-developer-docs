...
| honorForceAuthn       | Set to `true` to prompt the user to re-authenticate if the SAML request has the `ForceAuthn` attribute set to `true`. Set this property to `false` to ignore the `ForceAuthn` attribute in the SAML request. This property is set from the [SAML 2.0 App Integration Wizard](https://help.okta.com/okta_help.htm?type=oie&id=csh-apps-aiw-saml). You can set this property from the API if your org is enabled with the Enable Apps API additions feature.         | Boolean   | FALSE    | FALSE  |  |
| url       | The URL of the sign-in page for this app          | String   | FALSE    | FALSE  | [URL](http://tools.ietf.org/html/rfc3986) |
...
| honorForceAuthn       | Prompt user to re-authenticate if SP asks for it. This property is set from the [SAML 2.0 App Integration Wizard](https://help.okta.com/okta_help.htm?type=oie&id=csh-apps-aiw-saml). You can set this property from the API if your org is enabled with the Enable Apps API additions feature. | Boolean                                              | FALSE    | FALSE  |                                           |
