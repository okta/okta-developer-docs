...
| logo_uri                                    | URL string that references a logo for the client. When you create an app using the App Wizard in the UI, this value is used on the client consent dialog box during the client consent flows and is used as the logo on the application tile. When you create an app using the `/client` API, this value is used only on the client consent dialog box during the client consent flows.                | URL                                                                                            | TRUE       | FALSE    | FALSE      |
...
**Notes:**
  ...
  * These properties can also be configured in the App Wizard, on the **General** tab in the Admin Console: `tos_uri`, `policy_uri`, `logo_uri`, and `consent_method`. They can't be set using [the Dynamic Client Registration API](/docs/reference/api/oauth-clients/).
