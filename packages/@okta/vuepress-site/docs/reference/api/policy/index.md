...
| primaryFactor           | Indicates the primary factor used for the org ???  | String ??? What are the possible values for this property ?? `PASSWORD_IDP_ANY_FACTOR`, `PASSWORD_IDP` ??? | Yes, if `access` is set to `ALLOW`                        | N/A   |
