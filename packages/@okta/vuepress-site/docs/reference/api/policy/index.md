...
| primaryFactor <ApiLifecycle access="ie" /> | Indicates the primary factor used to establish a session for the org. Possible values: `PASSWORD_IDP_ANY_FACTOR` (any factor can be used to meet the authentication policy), `PASSWORD_IDP` (only a password can be used to meet the authentication policy)  | String | Yes, if `access` is set to `ALLOW`                        | N/A   |
