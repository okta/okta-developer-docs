...
> **Note:** For orgs with the Authenticator enrollment policy feature enabled, the new default authenticator enrollment policy created by Okta contains the `authenticators` property in the policy settings. Existing default authenticator enrollment policies, prior to enabling this feature, remains unchanged and still uses the `factors` property in its policy settings.
