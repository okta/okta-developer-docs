...
| primaryFactor <ApiLifecycle access="ie" /> | Indicates the primary factor used to establish a session for the org. Possible values: `PASSWORD_IDP_ANY_FACTOR` (any factor required by the app authentication policy can be used to establish a session), `PASSWORD_IDP` (a password must always be used to establish a session)  | String | Yes, if `access` is set to `ALLOW`                        | N/A   |
