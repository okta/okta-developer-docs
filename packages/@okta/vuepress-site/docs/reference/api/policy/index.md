...
| type        | Specifies the [type of Policy](#policy-types). Valid values: `OKTA_SIGN_ON`, `PASSWORD`, `MFA_ENROLL`, `OAUTH_AUTHORIZATION_POLICY`, `IDP_DISCOVERY`. <ApiLifecycle access="ie" /><br> <br>**Note:** Following policy types are only available as a part of the Okta Identity Engine. Please [contact support](mailto:dev-inquiries@okta.com) for further information: <br>`ACCESS_POLICY`, `PROFILE_ENROLLMENT` | String                                            | Yes      |                        |
...
| type          | Rule type. Valid values: `SIGN_ON` or `PASSWORD` or `MFA_ENROLL` or `IDP_DISCOVERY`. <ApiLifecycle access="ie" /><br> <br>**Note:** Following policy types are only available as a part of the Okta Identity Engine. Please [contact support](mailto:dev-inquiries@okta.com) for further information: <br>`ACCESS_POLICY` or `PROFILE_ENROLLMENT`  | String (Enum)                                  | Yes        |                        |
