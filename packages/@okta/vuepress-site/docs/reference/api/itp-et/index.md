...
| Risk            | Contains the level of risk for the current request (`LOW`, `MEDIUM`, or `HIGH`) and the reasons that contributed to the risk level. The `detectionName` key defines the risks monitored by Okta. The `level` key defines the current risk. The `issuer` defines the source of the risk detection. See [Detections](https://help.okta.com/okta_help.htm?type=oie&id=csh-detections).               | key-value pairs         |`{reasons=Anomalous Location, Anomalous Device, level=MEDIUM, detectionName=Suspected Session Hijacking}`<br><br> **Note:** If the risk level change is `LOW` to `LOW` or `NONE` to `LOW`, the `detectionName` key displays as `null` indicating no named risk detection.          |
