...
| Risk            | Contains the level of risk for the current request (`LOW`, `MEDIUM`, or `HIGH`) and the reasons that contributed to the risk level. The `detectionName` key defines the risks monitored by Okta. The `level` key defines the current risk. The `issuer` defines the source of the risk detection. See [Detections](https://help.okta.com/okta_help.htm?type=oie&id=csh-detections).               | key-value pairs         |`{reasons=Anomalous Location, Anomalous Device, level=MEDIUM, detectionName=Suspected Session Hijacking}`<br><br> If the risk level change is `LOW` to `LOW` or `LOW` to `NONE`, there is no value for the `detectionName` and displays in the system log as `null`.          |
