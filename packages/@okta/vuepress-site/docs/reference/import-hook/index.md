...
In the case where you don't want the hook to change the user import operation or update a user profile, you can return an empty response (or no response at all). The User Import Inline Hook only modifies user imports with a response using one of the [commands](#commands).
