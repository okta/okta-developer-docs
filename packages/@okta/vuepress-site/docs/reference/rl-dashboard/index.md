...
Display the desired bar graph by selecting **Top Requests by IP**, **Top Requests by API Token**, or **Top Requests by Application** (OAuth 2.0 application) in the dropdown menu. See [API rate limits by API token or OAuth 2.0 app](/docs/reference/rate-limits/#api-rate-limits-by-api-token-or-oauth-20-app).
