...
API endpoints in the dashboard that end with an asterisk (for example, `/api/v1/users/*`) generally refer to all calls that use that base schema and count towards the overall limit, unless the endpoint is specifically called out in the dashboard. It's also important to note the HTTP operation involved with each call as the same call with different operations can count towards different limits.
