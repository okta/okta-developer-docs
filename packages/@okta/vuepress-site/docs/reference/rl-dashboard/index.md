...
The dashboard enables you to view an endpoint, the current rate limit for each endpoint, and usage of the endpoint over a period of time. You can select the endpoint of interest and get additional details such as the endpoint's average usage and the top 10 IPs that are consuming the most traffic for the endpoint.
