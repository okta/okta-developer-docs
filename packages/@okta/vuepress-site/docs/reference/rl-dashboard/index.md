...
* You've hit a rate limit violation for an endpoint, and you receive a notification that contains a link to the rate limits dashboard for that endpoint so that you can investigate the violation. See [Endpoint rate limits and violations](#endpoint-rate-limits-and-violations).

## Rate limit monitoring

The overview of the rate limits dashboard enables you to browse and monitor rate limit usage over a period of time whenever you want to check on an endpoint. You can also configure the client rate limit settings and customize the threshold percentage for when a warning notification occurs.

The following describes typical uses of the functions in the Rate limits report:

* If the endpoint had rate limit violations in the past, you can proactively monitor that endpoint by checking its rate limits, any increase in traffic, and any abnormal activity. If you notice an increase in rate limit usage, you can start investigating the issue.
* If you're going to have an event and the endpoint is going to be affected, you can check the current traffic on the endpoint before the event. Then you can plan for traffic on the endpoint during the event and determine the rate limit capacity for it.
* If you want to control the warning threshold on traffic consumption for an endpoint, you can configure the rate limits settings and customize when your org should trigger a warning event. For instance, if you have CIAM and you've configured a [custom threshold](#warning-notification), a warning alert appears if you reach 90% of the rate limit. If you have Workforce, a warning alert appears if you reach 60% of the rate limit.
...
The **Settings** tab allows you to change the per client rate limit settings and customize the threshold for warning notifications when the endpoint's rate limit is exceeded. See [Change rate limit settings](#change-rate-limit-settings).

### Browse and monitor rate limit usage

#### Violation graphs

The Violations line graph plots the number of violations over the selected time period, which can be the last 24 hours, last 7 days, last month, or last 3 months. The Violations per endpoint bar graph shows the top ten number of violations by endpoint over the same time period.

You can hover over the line graph or bar graph to get more details. The details also include a link to the system log where you can view filtered events specific to that endpoint and time period.

#### APIs table

The APIs table shows each endpoint and its corresponding rate limit for your org. The rate limits include all overrides, dynamic scale, or Workforce multipliers. The table also shows the number of violations and warnings for each endpoint in the last hour, and includes a trendline of usage in the past hour.
...
Warnings, which appear if the threshold for the rate limit is reached, generate a system log event, and issue banner and email notifications. Before your endpoint hits its rate limit, you can change the threshold for when you want to be notified that your endpoint has consumed a high amount of traffic.

For example, you may want to customize the rate limit threshold when an endpoint normally has high traffic. In such a case, the threshold if set too low at 60% would generate too many warning notifications. If you customize the threshold to a higher percentage, such as 90%, the warning notifications wouldn't start until the endpoint's rate limit is at 90%.

## Endpoint rate limits and violations

The dashboard enables you to view an endpoint, the current rate limit for each endpoint, and usage of the endpoint over a period of time. You can select the endpoint of interest and get additional details such as the endpoint's average usage and the top 10 IPs that are consuming the most traffic for the endpoint. A warning banner appears at the top of the Admin console if the threshold for the rate limit is reached.
...
![Displays the email notification of a rate limit violation with the link to the endpoint's rate limit dashboard](/img/rl_dashboard1.png)
...
![Displays the banner in the dashboard that notifies you of a rate limit violation with the link to the endpoint's rate limit dashboard](/img/rl_dashboard2.png)
...
![Displays the event in the System Log that notifies you of a rate limit violation with the link to the endpoint's rate limit dashboard](/img/rl_dashboard3.png)
