...
* [Okta API endpoints and per-user limits](/docs/reference/rl-additional-limits/#okta-api-endpoints-and-per-user-limits): API endpoints that take username and password credentials, including the [Authentication API](/docs/reference/api/authn/), the [OAuth 2.0 resource owner password flow](/docs/guides/implement-password/), and [SMS and Call factor endpoints](/docs/reference/api/factors/#factor-lifecycle-operations), have a per-username rate limit to prevent brute force attacks with the user's password.
