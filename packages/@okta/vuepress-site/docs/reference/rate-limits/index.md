...
The Okta API rate limits are divided into three categories: end-user access, management, and other endpoints. Each category has a combined rate limit as well as individual rate limits per API endpoint. The rate limits vary by [service subscription](https://developer.okta.com/pricing/).
...
> * You can expand the Okta rate limits upon request. To learn how, see [Rate increase requests](/docs/reference/rl-additional-limits/#concurrent-rate-limits) and [DynamicScale rate limits](/docs/reference/rl-dynamic-scale/).
>
