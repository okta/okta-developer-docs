...
Okta provides rate limits for orgs based on the traffic that they expect to have. If your org experiences higher traffic than what is expected, this unplanned usage may potentially have an impact on end users. To ensure our customers are successful and to help minimize impact, Okta will not suspend usage above the rate limit (specifically for authentication and authorization flows). In the event that there is sustained usage above the purchased rate limit, Okta will require that you purchase an applicable offering that is in line with your usage. By making burst rate limits available, Okta provides peace of mind by ensuring an unplanned spike doesn't detrimentally affect the end user's experience.

In a typical use case scenario, when orgs exceed a default rate limit, they receive a system log warning event, then a burst event, and then a violation event. For example, an org has a rate limit of 600 requests per minute on the `/api/v1/authn` endpoint. That org would receive a warning at 360 requests per minute (60%) of 600. That org would get a burst notification when the endpoint hits 600 requests per minute and then the violation event when it hits 3000 requests all in the same minute.

Additionally, burst rate limits will typically apply on top of any rate limit increase that an org may have, such as [DyanmicScale](/docs/reference/rl-dynamic-scale/). For example, the default limit on `/api/v1/authn` is 600 requests per minute. If an org is expecting traffic to require 6000 requests per minute, the org would purchase DynamicScale 10x. The burst rate limit in this scenario would provide 5x coverage on top of the 6000 and ensure peace of mind for any unplanned spike in usage.
