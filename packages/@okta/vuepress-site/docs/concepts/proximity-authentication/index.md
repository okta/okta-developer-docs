...
* **Okta Verify**: Okta Verify is an app that's installed on the Windows device. It acts as the proximity agent and handles all NFC hardware interaction between the device, the reader, and the card.
  > **Note:** Only managed Okta Verify installations on Windows desktop devices can participate in NFC authentication, currently. See [Configure the NFC authenticator](https://help.okta.com/okta_help.htm?type=oie&id=nfc-configure) for prerequisites and setup steps.
