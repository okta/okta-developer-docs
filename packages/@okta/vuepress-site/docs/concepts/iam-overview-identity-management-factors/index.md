...
Your identity storage facilities maintain data about registered users, groups, devices, policies, apps, and other resources that's needed to authenticate users and authorize their access. Your apps and all elements of your solution rely on this data, so having distributed access to it is important.
...
- Organize users in hierarchical groups to administer users with common properties, and assist with assigning roles, permissions, and policies to groups that are inherited by their members.
...
- Provide APIs and SDKs that allow all elements of your IAM solution, apps, and other consumers to access identity data as authorized.
...
- User device registration and management, such as add a device, update device info, and remove an obsolete device. Consider allowing users to perform these tasks for themselves too, for the same reasons.
...
- Provisions for your admins to manage and support your users and customers. Examples might include those listed for customer admins before this for your domain, plus create, configure, and administer customer accounts.
...
- APIs and SDKs that allow all elements of your IAM solution, your apps, and other consumers, to access your identity functions as authorized.

- Customizable online help and other documentation of your solution to help new admins learn your admin features, and minimize your admin workload.

- Online technical documentation to help app developers use your APIs and SDKs.

## How Okta can help

Okta is a cloud-based IAM platform that is built around a centralized directory of user and customer profiles and other data. You can use its features to address the preceding identity challenges.
...
- Employ various means to authenticate users, such as multifactor authentication (MFA) and biometrics, to address a flexible set of needs.
...
- [Okta Identity Engine](https://help.okta.com/okta_help.htm?type=oie&id=csh-oie) has links for learning more about Identity Engine.

#### Okta Universal Directory

The *Universal Directory (UD)* is a full-featured directory that can mirror data in Active Directory (AD), LDAP, CSV directories, social media, and other external Identity Providers. This allows UD to serve as a single source of truth for all user data. Also, it allows you to combine user profiles from all of your identity sources and centralize user policy, profile, and password management.
...
- Get a real-time System Log to troubleshoot and address security issues and use pre-built reports to see how end users use apps and services. You can download or export your data to CSV files, sync it with Security Information and Event Management systems, or access it through its API.

- Admins can view and manage policies, reset passwords, and view and manage profiles of all users in their domains using the API or Admin Console.
...
A typical user profile contains attributes such as a user's first name, family name, username, and email address. A user is any person who is authorized to access your apps, and can be an employee, customer, partner, or other entity.
...
- [User profiles](https://developer.okta.com/docs/concepts/user-profiles/) describes Okta user profiles and profile types, user mappings, custom profile types, and how the Universal Directory handles them.
...
*Progressive profiling* allows you to add information to user profiles, or have Okta request more info, after users register. Use it to require less data when users register, and collect reliable user demographic data later. You can also use it to have Okta request more data if profile requirements change, such as if an app requires new app profile data.
...
- [Configuring Progressive Profiling for Your Custom Apps](https://www.okta.com/blog/2019/01/configuring-progressive-profiling-for-your-custom-apps/) explains how you can collect more data if profile requirements change.
...
An Okta organization (org) is a root object and a container for all other Okta objects. It contains users, groups, apps and other resources, and policy and configurations for your Okta environment.

Every org has users and apps. These are the only mandatory items that you must configure for your org to use Okta. You can create users in Okta, import users through directory integrations or app integrations. Apps are connections to public apps (such as Office 365) or proprietary apps (such as your own apps).
...
- [Okta organizations](/docs/concepts/okta-organizations/) describes org URLs, types, and GA and EA feature types, org federation, multiple-org configurations, and rate limits.

#### Okta groups

Groups help you simplify managing users who have common or shared traits. For example, you can create a group named Sales and grant group members access to the Sales Documentation folder on your org file server. This makes it easier for your sales team to access shared sales documentation. Group data typically resides in the directory. You can also limit the scope of app sign-in roles to a group. For example, you can configure policies to implement MFA for remote, temporary, or contract employees.

With Okta, you can define group membership in one directory and then use your groups in multiple connected systems. In on-premises systems, apps can connect to and query for groups from a central directory. Cloud apps often lack a common Active Directory, but Okta lets you use groups with these types of apps.

Learn more:

- [About groups](https://help.okta.com/okta_help.htm?type=oie&id=ext_Directory_Groups) describes groups and their use for policies, SAML provisioning, and use cases.
...
Use *Okta Lifecycle Management (OLM)* to automate user provisioning, updates, and deprovisioning of app access in response to user lifecycle events. For example, use it to automate new hire onboarding, role changes, and company exits.
...
- Create and maintain group rules that allow admins to set policies for group membership, app permissions, automated provisioning, and more.

- Push groups and their memberships to provisioned third-party apps.

- Automatically respond to user lifecycle events. For example, you can alert a user or change their status if they’re inactive for too long. Or alert them if their sign-in credentials are about to expire.
...
- Trigger any given flow with an app event (such as adding a user to an app in Okta), with a scheduled event, or when an API endpoint is called.
