...
When Okta calls an external service, it enforces a default timeout of three seconds. By default, Okta attempts two retries. A request isn't retried if the external service endpoint responds with a 2xx HTTP success code or the following 4xx error codes: 400 (Bad request), 404 (Not found), or 405 (Method not allowed). If the external service endpoint responds with a redirect, it isn't followed.
