...
* You can add connections to identity verification vendors (IDVs) which work like IdPs with a few key differences. IDVs verify your user’s identities by requiring them to submit a proof of identity. The proof of identity can be matching a user’s digital identity against a trusted data source. Or it can be requiring a user to take a selfie as a liveliness check. You can only use IDVs with the [Okta account management policy](/docs/guides/okta-account-management-policy/main/).
