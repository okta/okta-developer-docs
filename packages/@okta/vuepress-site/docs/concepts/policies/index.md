...
* [Post auth session evaluation](/docs/reference/api/policy/#continuous-access-evaluation-cae-policy): Post auth session evaluation monitors user sessions on your org to identify changes in session context. For Admin Console tasks and further information, see [Post auth session](https://help.okta.com/okta_help.htm?type=oie&id=csh-continuous-access-evaluation).
