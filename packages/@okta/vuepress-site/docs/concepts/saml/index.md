...
A SAML IdP generates a SAML response based on configuration that is mutually agreed upon by the IdP and the SP. After receiving the SAML assertion, the SP needs to validate that the assertion comes from a valid IdP and then parse the necessary information from the assertion: the username, attributes, etc.
