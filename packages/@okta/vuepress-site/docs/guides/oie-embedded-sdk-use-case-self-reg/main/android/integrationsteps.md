...
Create a form to capture the user's new account details.
...
### The user enters their profile data
...
2. `Authenticators` = List of [authenticators](https://github.com/okta/okta-idx-java/blob/master/api/src/main/java/com/okta/idx/sdk/api/client/Authenticator.java) (in this case, there’s only the password authenticator). <br>

   ```kotlin
   val authenticators = authenticationResponse.authenticators
   ```

After receiving the `AWAITING_AUTHENTICATOR_ENROLLMENT_SELECTION` status and the list of authenticators, you need to provide the user with a form to select the authenticator to enroll. In the following wireframe, there’s only one password authenticator to enroll.
...
### The user selects the authenticator to enroll
...
This request returns an `AuthenticationResponse` object with `AuthenticationStatus=AWAITING_AUTHENTICATOR_VERIFICATION`. Build a form for the user to enter their password in this authenticator verification step.
...
### Verify the authenticator and display other authenticators
...
### The user selects the email authenticator
...
If this request is successful, a code is sent to the user's email and `AuthenticationStatus=AWAITING_AUTHENTICATOR_VERIFICATION` is returned. Build a form to capture the code for this verification step.
...
### The user submits the email verification code
...
If the user decides to skip the optional factor, they are considered signed in since they have already verified the required factors. See [The user skips the phone authenticator](#the-user-skips-the-phone-authenticator) for the skip authenticator flow. If the user decides to select the optional factor, see [The user selects the phone authenticator](#the-user-selects-the-phone-authenticator) for the optional phone authenticator flow.
...
### Handle the phone authenticator options

There are two ways to handle the phone authenticator options:

* [The user skips the phone authenticator](#the-user-skips-the-phone-authenticator)
* [The user selects the phone authenticator](#the-user-selects-the-phone-authenticator)

#### The user skips the phone authenticator

If the user skips the phone authenticator enrollment, then call the `IDXAuthenticationWrapper.skipAuthenticatorEnrollment()` method. This method skips the authenticator enrollment.
...
#### The user selects the phone authenticator

In this option, the user selects the optional **Phone** authenticator for verification. Pass this selected authenticator to the `IDXAuthenticationWrapper.selectAuthenticator()` method.
...
This status indicates that the user needs to provide other information about their authenticator. The user must then enter their phone number. Depending on how you handle the phone authenticator, the SMS verification code is sent automatically. Or the user enters their phone number and selects either the SMS or the Voice Verify method.

##### The user enters their phone number and the SMS verification code is sent automatically
...
Build a form to capture the user's SMS verification code.
...
##### The user enters their phone number and selects the verification method
...
![A form that asks users to choose their phone verification method with SMS and Voice options and a next button](/img/wireframes/choose-phone-verification-method-form.png)
...
Build a form to capture the user's verification code.
