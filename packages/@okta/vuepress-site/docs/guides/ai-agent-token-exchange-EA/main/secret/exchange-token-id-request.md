...
In this step, the AI agent sends a `POST` request to the Okta org authorization server's `/token` endpoint to exchange the token for the secret credentials.

> **Note:** This request example uses an ID token as the subject token type.
...
&resource=orn:okta:pam:00osvp3k63Qlcc9wUS9a:secrets:3ad38bbd-c057-4a69-9d8d-06b8ca86f411
