...
The `authenticate()` method returns a `transaction` object with a `status` property indicating the current state of the sign-in flow. Handle the returned `IdxStatus` value accordingly:

#### Success status

When the user correctly enters their password, the `IdxStatus` value equals `IdxStatus.SUCCESS`. To save the tokens retrieved from the response for future requests, call `tokenManager.setTokens()` and then redirect the user back to the home page. The user is now signed in.
