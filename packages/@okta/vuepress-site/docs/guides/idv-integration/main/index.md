...
| redirect_uri          | The URI where the response is sent after the user completes the IDV flow. <br></br> Ensure that the URI is located in a secure domain that you own. If you're using a custom domain, include that value in the URI. For example, if your Okta subdomain is called `company`, then the URL would be: `company.okta.com.` If you’ve configured a custom domain in your Okta org, use that value to construct your redirect URI, such as `login.company.com.` | String     | String   |
