...
* **Application context**: <ApiLifecycle access="ea" /> To send the app name (`app_name`) and app ID (`app_instancje_id`) to the IdP, select the **Send Okta application context** checkbox. Okta sends the `app_instance_id` and `app_name` parameters to the IdP. The IdP returns the same information as claims in the ID token in the response to Okta. Use application context to help external IdPs make more informed, context-aware authentication decisions.
