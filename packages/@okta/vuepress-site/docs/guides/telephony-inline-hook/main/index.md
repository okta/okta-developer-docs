...
* [File a ticket with Twilio Support](https://help.twilio.com/submit?subject=Enable+Custom+Code+for+Verify+Service&description=I+need+to+file+a+ticket+to+enable+custom+code+for+the+Verify+service.) to enable the [Custom Code feature](https://www.twilio.com/docs/verify/api/customization-options) for Verify SMS and Voice OTPs. Include your Verify Service SID in the request.
