...
* If you only want to support direct SSO to your application (i.e. the integration is not launched from the Okta End-User Dashboard), then:
  1. Enter one or more **Login redirect URI** values where Okta will send OAuth responses.
  1. Set the **Login initiated by** drop down menu to **App Only**.
  1. Leave all the remaining entries at their default values.

* If you want to support launching your application from the Okta dashboard:
  1. Enter one or more **Login redirect URI** values where Okta will send OAuth responses.
  2. Optionally, enter one or more **Logout redirect URIs** where Okta will send relying-party initiated sign-out requests.
  ...
  5. Select the appropriate **Login flow** option. If you choose **Send ID Token directly to app (Okta Simplified)**, you can also choose the **Scopes** associated with the login flow.
...
* In the **Client Credentials** section, you can select a **Client authentication** type:
  ...
  * **Use Client Authentication** &mdash; This option is not recommended for distributed native applications. A client secret is embedded in the client and sent with requests to prove the client's identity.
...
1. Select from among the different grant type options.
1. If you only want to support direct login to your integration (not launched from the Okta End-User Dashboard), then you only need to set the **Login initiated by**  to App Only, enter one or more **Login redirect URI** values and you can leave all the remaining entries at their default values.
...
* If you want to prompt your user to approve the integration access to specified resources, check the **Require consent** box. Alternatively, you can set up the consent for a scope in your custom authorization, as described in the [Create Scopes](https://help.okta.com/en/prod/okta_help_CSH.htm#create-scopes) section of the API Access Management documentation.
