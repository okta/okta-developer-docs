...
> **Note:** To locate the `clientId` for an app, from the Admin Console open the **Applications** menu and then select **Applications**. Select the App that you need the `clientId` for, select the **General** tab, and then in the **Client Credentials** section, copy the ID from the **Client ID** box.
