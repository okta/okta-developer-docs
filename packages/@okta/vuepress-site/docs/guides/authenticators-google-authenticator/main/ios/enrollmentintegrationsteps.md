...
The authenticators related to the current request are in the `authenticators` property of the remediation. The `Authenticator` class represents an authenticator and includes a a `key` property that identifies it, and a display name for the UI in the `displayName` property.

Associated information such as the QR Code and shared secret are accessed using capabilities.
...
typealias AuthenticatorCode = (codeField: Remediation.Form.Field,
    ...
    func authenticatorCodeField() -> AuthenticatorCode? {
        guard let remediation = currentRemediation,
              let codeField = remediation.form.fields.filter({ $0.name == "credentials"}).first,
              let entryField = codeField.form?.fields.filter({ $0.name == "passcode"}).first else {
                  return nil
              }
