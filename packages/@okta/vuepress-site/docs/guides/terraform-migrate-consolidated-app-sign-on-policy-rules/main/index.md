...
* Administrative access with permissions to manage policies and applications
...
The consolidated `okta_app_signon_policy_rules` resource prevents this by managing all rules for a policy as a single atomic unit, ensuring they are always applied in the correct, intended order.
...
Note: We recommend Option B because it is version-controlled. This approach allows your team to review the changes in a pull request and use the standard `terraform plan` and `apply` workflow. It also prevents errors that can occur when running manual CLI commands in the wrong order or workspace.
...
## Understanding "Positional Diffs"

You might see names "swapping" in the plan (for example, name: "Rule2" -> "Rule1"). This is usually just a display artifact. Terraform compares lists by position rather than by name.

* When it's safe: If the settings (like status or constraints) match the correct rule name in your code, you can safely apply.

* When to double-check: If you see configuration details moving to the wrong rule name, ensure your code names match the names in Okta exactly.
...
* Allow for priority gaps: Okta rules do not require consecutive numbers. You can use priorities like 1, 3, and 5 without issue.
* Manage changes easily:
  * To add a rule: Add a new `rule` block with a unique name.
