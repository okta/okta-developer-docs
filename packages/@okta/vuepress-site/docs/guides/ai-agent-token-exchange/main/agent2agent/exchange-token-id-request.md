...
In this step, the AI agent sends a `POST` request to the Okta org authorization server's `/token` endpoint to exchange the ID or access token for the ID-JAG resource token.
...
&subject_token=eyJraWQiOiJQLVgxeC1ITWtuSThPS0lUeE5TWVl...
...
```
   ...
   audience: https://sharper.trexcloud.com/oauth2/aus9jef27qggJqmEj0g7
   resource: https://approval-agent.sharper.trexcloud.com
   scope: agent.invoke
   client_assertion_type: urn:ietf:params:oauth:client-assertion-type:jwt-bearer
