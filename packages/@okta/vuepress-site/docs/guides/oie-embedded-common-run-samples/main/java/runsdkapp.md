...
1. Obtain app-specific configuration values (such as `${yourAppClientId}`, `${yourClientSecret}`, `${yourOktaDomain}`) from the app integration you've created in [Create new application](/docs/guides/oie-embedded-common-org-setup/java/main/#create-a-new-application).
...
1. Set the app-specific environment variables:
   ...
   export OKTA_IDX_CLIENTID=${yourAppClientId}
   export OKTA_IDX_CLIENTSECRET=${yourClientSecret}
