...
1. Obtain app-specific configuration values (such as `${yourAppClientId}`, `${yourClientSecret}`, `${yourOktaDomain}`) from the [app integration](/docs/guides/oie-embedded-common-org-setup/java/main/#create-a-new-application) you've created in Okta.
...
1. Set the app-specific environment variables:
   ...
   export OKTA_OAUTH2_CLIENTID=${yourAppClientId}
