...
* Any SCIM [tenant settings](#tenant-settings) that you define in the OIN Wizard are considered [application properties](/docs/reference/okta-expression-language/#application-properties). They have an `app.` prefix when you reference them in Expression Language. For example, if your tenant variable name is `subdomain`, then you can reference that variable using `app.subdomain`.
