...
| **Testing instructions** | Include any other information that you think the OIN analyst needs to know about your integration for testing (such as the admin account or the testing configuration). You can also provide instructions on how to add test user accounts. <br>For SCIM integrations, specify how to access the SCIM server for testing. If your test SCIM server is different than your production SCIM server, specify the test server base URL. Provide instructions on how to obtain credentials (API token or OAuth 2.0 properties) to access your SCIM server. The OIN team uses these test SCIM server credentials for [QA testing](/docs/guides/submit-app-overview/#understand-the-submission-review-process). |
