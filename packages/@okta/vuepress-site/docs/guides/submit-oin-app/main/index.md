...
| **Testing instructions** | Include information that the OIN team needs to know about your integration for testing (such as the admin account or the testing configuration). You can also provide instructions on how to add test user accounts. <br>For SCIM integrations, specify how to access the SCIM server for testing. Provide the SCIM server base URL and instructions on how to obtain credentials (API token or OAuth 2.0 properties) to access the server. The OIN team uses these SCIM server credentials for [QA testing](/docs/guides/submit-app-overview/#understand-the-submission-review-process). |
