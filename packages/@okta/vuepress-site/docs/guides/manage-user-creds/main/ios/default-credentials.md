...
* The `Credential` class sends an event (for example, `Notification.Name` type) whenever the default credential is set or removed. You can listen for this event to dynamically update your UI, such as transitioning from a sign-in screen to a signed-in view.
