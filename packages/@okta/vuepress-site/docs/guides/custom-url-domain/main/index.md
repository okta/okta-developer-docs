...
* **DomainKeys Identified Mail (DKIM):** [DKIM](https://www.cloudflare.com/en-ca/learning/dns/dns-records/dns-dkim-record/) adds a cryptographic signature to emails to verify that the message wasn't altered in transit. Generate DKIM keys with your email provider and add the resulting TXT records to your DNS configuration. For example, [Google Workspace provides DKIM keys](https://knowledge.workspace.google.com/admin/security/set-up-dkim) that you can add to your DNS records.
