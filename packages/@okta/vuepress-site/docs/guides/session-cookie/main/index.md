...
After your sign-in flow is complete you can launch an Okta app for the user. You can launch the app with an [embed link](https://developer.okta.com/docs/api/openapi/okta-management/management/tag/UserResources/#tag/UserResources/operation/listAppLinks) that contains the session token as a query parameter: `sessionToken`.
