...
In Classic Engine, you enable a registration inline hook by selecting it from the **Extension** dropdown menu on the **Self-Service Registration** page (**Directory** > **Self-Service Registration**). That page and the Classic self-service registration feature aren't used in Identity Engine.
...
app.post('/registrationHook', async (request, response) => {
  ...
  if (request.body.requestType === 'self.service.registration') {
    ...
    const emailRegistration = request.body.data.userProfile['login'].split('@');
