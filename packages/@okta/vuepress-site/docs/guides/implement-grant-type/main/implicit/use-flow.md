...
To get the tokens, your app redirects the user to your [authorization server's](/docs/concepts/auth-servers/) `/authorize` endpoint. If you're using the default custom authorization server, then your request URL would look something like this:
...
* `scope` is `openid`, which is required, but other scopes can be requested. See the **Create Scopes** section of the [Create an authorization server guide](/docs/guides/customize-authz-server/main/#create-scopes).
