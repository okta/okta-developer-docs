...
> **Note:** The examples in this guide use the [Implicit flow](/docs/concepts/oauth-openid/#implicit-flow) flow for quick testing. In the following example we are requesting only `id_token` as the `response_type` value [fat ID token](/docs/concepts/api-access-management/#tokens-and-scopes), which means that the ID token is the only token included in the response. The ID token contains any groups assigned to the user that signs in when you include the `groups` scope in the request.

If you want a Groups claim present in the ID token when you request both an ID token and access token, use a <GuideLink link="../add-groups-claim-custom-as">Custom Authorization Server</GuideLink>.
