...
> **Note**: If you have a new Integrator Free Plan org and plan to use the `default` custom authorization server, check that it has an access policy. Add an access policy if it's not there. See [Create access policies](https://help.okta.com/okta_help.htm?type=oie&id=ext-create-access-policies).
...
1. Enter the scope name (**api:access:read**) in the dialog that appears.
1. Click **Create**.
