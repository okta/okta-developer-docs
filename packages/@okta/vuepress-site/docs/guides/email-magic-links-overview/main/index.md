...
* Understand the differences between Email Magic Links (EML) and one-time passcode (OTP).
...
> **Note**: EML is only supported in OIDC-based app integrations.
...
* A user can only access an app by clicking a magic link on a device that already has access to that app.
...
Any magic link URL pointing to your app contains two query parameters:
...
3. Makes any other checks that you deem necessary to ensure that the user is functional in the same browser on the same device
...
If your app uses the embedded Sign-In Widget to authenticate the user, pass `otp` and `state` to it as you instantiate it on the page.
...
The code for your endpoint must check that the user is functional from the same device and browser. The Sign-In Widget takes care of the rest. The sign in with email only guide covers how to do this.
...
There are two ways to set your app's magic links to the endpoint that you created:

* You can set your app's Email Verification Experience (EVE) value to the endpoint's URL.
...
![Flow diagram showing the interaction between an app and Identity Engine when using the Verification Experience setting](/img/advanced-use-cases/email-magic-links-overview-experience-flow.png)
...
To point the magic link directly to the endpoint in your app, you must replace `${emailAuthenticationLink}` with your custom URL that includes `otp` and `state` as query parameters:
...
When a user clicks the magic link based on a customized email template, their browser is redirected straight to your app's endpoint:
