...
* **Okta authentication (platform-agnostic).** The AI agent performs a two-step token exchange: it turns the user's `id_token` into an Identity Assertion JWT authorization grant (ID-JAG) at the org authorization server, then turns the ID-JAG into a scoped `access_token` at a custom authorization server. This logic is identical for every AI agent, so you implement it once as a reusable module.
