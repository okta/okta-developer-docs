...
2. If the credentials in the token request are accurate, Okta responds with an access token.
   ...
   The resource and action scopes allowed for the access token is configured by you when you [register your API service integration](#register-your-api-service-integration) through the OIN Manager. In addition, the token request must contain allowed scopes for your API service integration. See [Scope selection](#scope-selection).
...
> **Note:** The test install and authorize screens, from the OIN Manager test option, are exactly the same screens your customers will experience when your service integration is in the OIN. After your integration is published in the OIN catalog, your integration appears in the **Applications** > **API Service Integrations** page of your customer's Admin Console.
...
1. Make a request to the [/token](/docs/reference/api/oidc/#token) endpoint with these query parameters:
   ...
   * `scope` must be at least one of the [Okta API scopes](/docs/guides/implement-oauth-for-okta/main/#scopes-and-supported-endpoints)
...
The fields are automatically saved in your submission.
