...
> **Note:** The `${yourAuthorizationServerId}` for the default server is `default`.
...
To verify that your server was created and has the expected configuration values, you can send an API request to the server's OpenID Connect Metadata URI: `https://${yourOktaDomain}/oauth2/${yourAuthorizationServerId}/.well-known/openid-configuration` using an HTTP client or by typing the URI inside of a browser. This returns information about the OpenID configuration of your authorization server.
...
`/api/v1/authorizationServers/${yourAuthorizationServerId}/scopes`
...
`/api/v1/authorizationServers/${yourAuthorizationServerId}/claims`
...
First, you need the authorization server's authorization endpoint, which you can retrieve using the server's Metadata URI: `https://${yourOktaDomain}/oauth2/${yourAuthorizationServerId}/.well-known/openid-configuration`.
...
`https://${yourOktaDomain}/oauth2/${yourAuthorizationServerId}/v1/authorize?client_id=examplefa39J4jXdcCwWA&response_type=id_token&response_mode=fragment&scope=openid%20profile&redirect_uri=https%3A%2F%2F{yourSignInRedirectUri}&state=WM6D&nonce=YsG76jo`
...
`https://{yourSignInRedirectUri}/#id_token=eyJhbGciOiJSUzI1NiIsImtpZCI6ImluZUdjZVQ4SzB1SnZyWGVUX082WnZLQlB2RFowO[...]z7UvPoMEIjuBTH-zNkTS5T8mGbY8y7532VeWKA&state=WM6D`
