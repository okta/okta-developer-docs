...
In Okta Classic Engine when a user doesn't have a session, the more restrictive policy (Okta sign-on policy and application sign-on policy) or assurance requirement (acr_values parameter) is evaluated first, then the second most restrictive and so on. When a user already has a session, the application sign-on policy is evaluated first. Then, the acr_values parameter in the request is evaluated after.
