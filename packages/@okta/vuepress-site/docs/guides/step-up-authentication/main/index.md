...
Okta's [redirect and embedded](/docs/concepts/redirect-vs-embedded/#redirect-authentication) deployment models support the use of the `acr_values` parameter. The parameter works with any OpenID Connect application, such as web, native, or SPA, and it’s supported by both the [Okta Org Authorization Server and custom authorization servers](/docs/concepts/auth-servers/).
