...
1. Under **User access and authentication**, check **Allow users to access this agent**, then select one of:
   ...
   * **Select an existing app:** To select an existing app integration instance in your org to bind to this AI agent. You can only select a custom SSO app that you created previously to bind to the AI agent. Use this option if your agentic app uses SAML for SSO. See [Create SAML app integrations](https://help.okta.com/oie/en-us/content/topics/apps/apps_app_integration_wizard_saml.htm).
        > **Note:** Currently, you can only select an exisitng OIN SAML app instance. You can't select an existing OIN OIDC app instance.

   The app you select in the **User access** tab represents the requesting app for the XAA flow. It allows your users to sign in to the agentic app through Okta, and the agentic app to access resource apps on behalf of the signed in user.
...
1. On the **Client registration** tab, select a client registration method:
   ...
   * **Public/private key**: Recommended for AI agents that have builder-managed key pairs.
      ...
      1. Click **Copy to clipboard** and use the private key in your AI agent app (the requesting app).
      1. Click **Done**.
...
If your org is subscribed to Okta for AI Agents, the **Delegations** tab has been renamed to **User access** on the AI agent page.
...
If you've previously configured an AI agent with app delegation links, your delegation links now appear as user access apps. The following behavior applies to the **User access** configuration model:
...
* If you have multiple app delegation links previously, you need to delete all of the migrated user access apps and recreate only one user access app.
* If you want to modify your previous app delegation link configuration, you need to delete your exising user access app configuration and recreate a new one.

> **Note:** Error message appears if your AI agent was configured using delegation links.

### Recreate your AI agent user access app
