...
meta:
  - name: description
    content: Inventory your Classic Engine dependencies across your apps and map each one to the Identity Engine path that should replace it.
---

<ApiLifecycle access="ie" />

After you upgrade from Classic Engine to Okta Identity Engine (Identity Engine), many applications keep running authentication flows built on Classic Engine patterns. These include the Classic Authentication API, the Factors API, the Sessions API, and older self-hosted Sign-In Widgets.
...
This guide helps you inventory those Classic Engine dependencies across your applications. It also helps you map each one to the Identity Engine path that should replace it.
...
**Note:** An Identity Engine upgrade doesn't automatically rewrite your application code. Sign-in can keep working on Classic patterns, so a code and configuration audit is the only reliable way to know where Classic dependencies remain.
...
The rows above match the Classic patterns most teams encounter. Add a row only when you find a Classic dependency that isn't represented here, then map it to the closest supported Identity Engine path.

## How to choose the Identity Engine destination

Once you know which Classic patterns each app uses, decide where that app should go in Identity Engine. For a ranked comparison of the deployment models and their trade-offs, see Choose your OIE authentication modernization approach.
<!-- TODO(OKTA-1216283 follow-up): link "Choose your OIE authentication modernization approach" to /docs/guides/choose-oie-authentication-approach/main/ when that guide publishes. -->

## Audit output template

Record one row per application, or per integration if an app has more than one Okta integration. Copy this worksheet into a spreadsheet or tracker and fill it in as you audit.
...
* Every active app that uses Okta for authentication has been reviewed and recorded.
...
* Every Sign-In Widget version and SDK version is identified and mapped to an upgrade or replacement path.
* Factor-related logic is mapped to Identity Engine authenticator policies, and session-related assumptions are checked against the Identity Engine session model.
* Open questions are assigned to the right reviewers. App owners review code, administrators review policy and app settings, and security or architecture reviewers handle risk decisions.
