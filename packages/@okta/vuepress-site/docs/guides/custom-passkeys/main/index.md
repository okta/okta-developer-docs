...
* A valid and certified [custom domain](#set-up-a-domain-to-use-as-an-rp-id)
...
### Set up a domain to use as an RP ID

Your custom domain can be either a subdomain or a root domain. You can use either as the RP ID for passkeys, but the verification method differs.

| RP ID type | Example | Domain verification method |
|------------|---------|----------------------------|
| Custom subdomain | `login.globex.com` | TXT and CNAME records must be verified during [custom domain setup](/docs/guides/custom-url-domain/). No additional API verification needed. |
| Custom root domain | `globex.com` | Only the TXT record must be verified by using the Verify a Relying Party ID domain [endpoint](https://developer.okta.com/docs/api/openapi/okta-management/management/tag/Authenticator/#tag/Authenticator/operation/verifyRpIdDomain). The TXT record value is returned when you [set the RP ID](#update-the-rp-id-for-the-passkeys-authenticator). |
...
> **Note:** Adding `globex.okta.com` to the associated domains list allows passkeys registered under the `globex.com` RP ID to be presented when users visit `globex.okta.com`. It doesn't make passkeys that are previously registered under the `okta.com` RP ID (credentials enrolled at `globex.okta.com` before this configuration, for example), work on `globex.com` or its subdomains. Users with passkeys registered under the previous RP ID must re-enroll them.

Before you begin, ensure that you've done the following:

* [Set `globex.com` as your primary RP ID](#update-the-rp-id-for-the-passkeys-authenticator). Setting `globex.com` as your primary RP ID enables passkeys to work for all its subdomains (`login.globex.com`, for example).
