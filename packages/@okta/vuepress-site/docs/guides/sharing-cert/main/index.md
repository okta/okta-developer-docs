...
[Application Key Credential objects](/docs/references/api/apps/#application-key-credential-object) contain an opaque key ID (`kid`) and a corresponding certificate. To use the [API](/docs/references/api/apps/#clone-application-key-credential) to share application key credentials between apps, you need to clone an application key credential. You create and use a new credential in one app, and then share and update the credential in another app.

Sharing certificates is useful for Okta orgs that have apps with [sign-on modes](/docs/references/api/apps/#sign-on-modes) such as `SAML_2_0`, `SAML_1_1`, or `WS_FEDERATION`.
...
* [Application sign-on modes](/docs/references/api/apps/#sign-on-modes)
* [Generate a new application key credential](/docs/references/api/apps/#generate-new-application-key-credential)
* [Clone an application key credential](/docs/references/api/apps/#clone-application-key-credential)