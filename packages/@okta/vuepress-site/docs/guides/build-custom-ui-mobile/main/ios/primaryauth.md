...
There are several ways that you can build UI flows in iOS apps. You can use Model-View-ViewModel (MVVM), Viper, or massive view controllers. How you manage transitions between view controllers is up to you, however Okta recommends the use of the [flow coordinator pattern](https://medium.com/@dkw5877/flow-coordinators-333ed64f3dd). Your flow coordinator instance handles the status updates and manages UI navigation. Okta has also implemented this pattern in our [sample app](https://github.com/okta/samples-ios/tree/master).

For example, your app could have the following view controllers:
- `LoginViewController` - responsible for reading the username and password information from a user and initiating the authentication flow through `OktaAuthSdk`
...
The flow coordinator decides which view controller to show next based on the status that is received from the server. For example, if the status is `OktaAuthStatusPasswordChange`, then the flow coordinator shows `PasswordChangeViewController` and injects the status as a dependency.

### Primary authentication flow

With the primary authentication flow (no MFA, no password management, and so on), you typically need `LoginViewController`. `LoginViewController` is your root view controller that expects the username and password to be input by the user. Let's assume that your view controller has the following UI outlets: `userNameTextField`, `passwordTextField`, and `signInButton`. Then, the handler for the tap on `signInButton` could be:
